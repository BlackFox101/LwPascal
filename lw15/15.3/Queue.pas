...
PROCEDURE Copy(VAR FIn, FOut: TEXT);
 {копириует строку из FIn в FOut без RESET или REWRITE;
  таким образом FIn должен быть готов для чтения,а FOut для записи,
  но строки прошлого у этих файлов могут быть не пусты }
VAR
  Ch: CHAR;
BEGIN {Copy}
  WHILE NOT EOLN(FIn)
